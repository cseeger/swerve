defmodule SwerveWeb.UserAuthTest do
  use SwerveWeb.ConnCase, async: true

  alias Swerve.Accounts
  alias SwerveWeb.UserAuth
  import Swerve.AccountsFixtures

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SwerveWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == SwerveWeb.Router.Helpers.link_index_path(conn, :index)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SwerveWeb.Endpoint.subscribe(live_socket_id)
